...
`bin/cw.js` is the entry point — it spawns `tsx` to run `src/index.ts` directly, so there is no build step. TypeScript is never compiled to JS in normal usage.
...
**Config** is stored at `~/.config/cw/config.json` and managed by `lib/config-store.ts`. The `Preset.org` field is optional — omitting it means the preset uses the authenticated user's own repos.
